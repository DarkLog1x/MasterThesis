...
IP: ip_internal_auth - publickey
    \end{lstlisting}
\end{mdframed}
The file is read in line by line. The system will then parse each line, splitting each line via the $:$ and $-$ character. The first part before the $:$, is matched with the different scanning modules. Thus allowing each one to only recognize the lines of the configuration meant for its task. The part after the $:$ represents what the scan will be looking for. In the case of the first test, this will be port 22. The next command looks at what the expected result should be.  If the result of the module scan does not match the expected output, a message is created and sent to the communication mechanism, in this case Slack. In the example above port 22 is expected open with the version of "7.2p2 Ubuntu 4ubuntu2.1". The version information is additional information that the Nmap scan can take in and use. This information needs to match the output that Nmap produces as it will be compared inside the Nmap scanning module.
