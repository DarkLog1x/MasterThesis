...
Cloud computing has some distinct differences from regular server computing. Regular server computing recourses are tuned to stay on line for long periods of time, with little downtime. Deployments occur overtime and require extensive prep work.  As with cloud computing the main focus is to run a service or provide a computational host. However with this model, services can be continually deployed, systems can be brought down and enabled with little to no downtime. A server can be automatically expanded to meet unusual load or shrunk to save costs.  Due to this difference systems in the cloud differ from typical.
